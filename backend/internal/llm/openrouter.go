...
// StreamChatWithHistory sends a chat request with conversation history and streams the response
func StreamChatWithHistory(messages []Message, callback StreamCallback) error {
	apiKey := GetAPIKey()
	if apiKey == "" {
		return fmt.Errorf("OPENROUTER_API_KEY not configured")
	}
